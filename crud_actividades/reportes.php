...
// Buscar actividad de manera segura
$search = isset($_GET['search']) ? trim($_GET['search']) : '';
$sql = "SELECT * FROM reportes WHERE actividad LIKE ? ORDER BY fecha_inicio DESC";
...
<div class="container mt-4">
    <div class="d-flex justify-content-between mb-3">
        ...
        <form class="d-flex">
            <input class="form-control me-2" type="search" placeholder="Buscar actividad" name="search" value="<?php echo htmlspecialchars($search); ?>">
        ...
        <tbody>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <button class="btn btn-warning btn-sm" onclick="openEditModal(<?php echo htmlspecialchars(json_encode($row)); ?>)">Editar</button>
                        <a href="eliminar.php?id=<?php echo $row['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Seguro que deseas eliminar esta actividad?');">Eliminar</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
...
function drawChart() {
    fetch('data.php')
        ...
        .then(data => {
            ...
            data.forEach(item => {
                let startDate = new Date(item.fecha_inicio); // Convierte la fecha de inicio a un objeto Date
                let endDate = new Date(item.fecha_fin); // Convierte la fecha de fin a un objeto Date
                let duration = (endDate - startDate) / (1000 * 60 * 60 * 24); // Calcula la duración en días
